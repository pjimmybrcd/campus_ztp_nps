...
class SessionAction(Action):
    def __init__(self, config):
        ...
        if key:
            self._ruckus_controller_password = symmetric_decrypt(crypto_key, key.value)

        key = client.keys.get_by_name('campus_ztp.ruckus_controller_enable_username')
        if key:
            self._ruckus_controller_enable_username = key.value

        key = client.keys.get_by_name('campus_ztp.ruckus_controller_enable_password')
        if key:
            self._ruckus_controller_enable_password = symmetric_decrypt(crypto_key, key.value)
