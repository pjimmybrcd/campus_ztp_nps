...
class Ruckus_Controller_Session(object):
    ...
    def __init__(self, hostname, username='', password='', enable_username='', enable_password=''):
        ...
        self.session = None
        self.session_state = Ruckus_Controller_Session.NO_SESSION
        self.session_lf = ''
        self.session_prompt = ''
    ...
    def login(self):
        ...
        self.session.logfile = open('/var/log/campus_ztp_ruckus_sshlog', 'w')
        ...
        if i == 0:
            ...
            return False
        if i == 1:
            self.session_state = Ruckus_Controller_Session.SESSION_AVAILABLE
        if i == 2:
            self.session_state = Ruckus_Controller_Session.PRIVILEDGE_MODE
        if i == 3:
            sys.stderr.write("Failed to connect!\r\n")
            return False
    ...
    def enter_enable_mode(self):
        ''' enters enable mode '''
        if self.session_state == Ruckus_Controller_Session.SESSION_AVAILABLE:
            prompt = self.ansi_escape.sub('',self.session_prompt)
            ...
            self.session_state = Ruckus_Controller_Session.PRIVILEDGE_MODE
            return True
        if self.session_state == Ruckus_Controller_Session.PRIVILEDGE_MODE:
            return True
    ...
    def exit_configuration_mode(self):
        ...
        if self.session_state == Ruckus_Controller_Session.CONFIGURATION_MODE:
    ...
    def page_on(self):
        if self.session_state == Ruckus_Controller_Session.PRIVILEDGE_MODE:
            prompt = "%s#" % self.ansi_escape.sub('',self.session_prompt)
            self.sendline(' ')
    ...
    def send_line(self, line):
        ...
        if self.session_state == Ruckus_Controller_Session.CONFIGURATION_MODE:
            ...
            prompt = r'%s\((.).+\)#' % self.ansi_escape.sub('',self.session_prompt)
        ...
        c = self.session.expect(['.*\[yes/no\]', '%s#' % prompt, pexpect.EOF, pexpect.TIMEOUT], timeout=5)
        if c==0:
